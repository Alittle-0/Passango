...
function Modal({ isOpen, onClose, title, user, setUser }) {
  ...
  const handleAvatarUpdate = async (avatarData) => {
    try {
      // Update user state with new avatar data
      const updatedUser = {
        ...user,
        avatar: {
          data: avatarData.avatar.data,
          contentType: avatarData.avatar.contentType,
        },
      };
      ...
      // Force update header avatar by dispatching a custom event
      window.dispatchEvent(new Event("userUpdated"));

      // Update preview
      setAvatarPreview(
        `data:${avatarData.avatar.contentType};base64,${avatarData.avatar.data}`
      );
